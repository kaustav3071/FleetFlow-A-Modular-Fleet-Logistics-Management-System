      ...
      <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {licenseCategoryOptions.map(opt => (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium border transition-colors ${
                  form.licenseCategory.includes(opt.value)
                    ? 'bg-brand-500 border-brand-500 text-surface-900'
                    : 'bg-surface-800 border-surface-600 text-surface-300 hover:border-brand-500/50'
                }`}
              >
                {opt.label}
              </button>
        ...
        <div className="space-y-1.5">
          ...
          <label className="flex items-center gap-3 px-4 py-3 rounded-xl border border-dashed border-surface-600 hover:border-brand-500/50 transition-colors cursor-pointer bg-surface-800/30">
            ...
            <span className="text-sm text-surface-400">{avatar ? avatar.name : 'Click to upload photo'}</span>
